...
export default function Portfolio() {
  ...
  return (
    <section ref={targetRef} id="case-studies" className="relative h-[250vh] md:h-[180vh] bg-primary">
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        <div className="px-6 lg:px-12 mb-12 max-w-7xl mx-auto w-full">
          <motion.div
        ...
        <motion.div style={{ x }} className="hidden md:flex gap-12 px-12">
          {caseStudies.map((study) => (
            <ProjectCard key={study.id} study={study} />
          ))}

          {/* Last Card: Call to Action */}
          <motion.div className="h-[60vh] w-[600px] shrink-0 flex items-center justify-center">
        ...
        <div className="flex md:hidden gap-6 px-6 overflow-x-auto pb-10 no-scrollbar snap-x snap-mandatory">
          {caseStudies.map((study) => (
            <div key={study.id} className="snap-center">
              <ProjectCard study={study} isMobile />
            </div>
          ))}
          <div className="snap-center shrink-0 w-[85vw] flex items-center justify-center">
            <a href="#/case-studies" className="group text-center space-y-6">
              <h3 className="text-4xl font-black uppercase tracking-tighter text-secondary leading-none">
                See <br /> All <span className="text-accent">Results</span>
              </h3>
              <div className="w-16 h-16 rounded-full border-2 border-accent/20 flex items-center justify-center mx-auto">
...
function ProjectCard({ study, isMobile = false }: { study: any; isMobile?: boolean }) {
  return (
    <motion.div
      className={`group relative ${isMobile ? 'h-[50vh] w-[85vw]' : 'h-[60vh] w-[600px]'} shrink-0`}
    >
